...
class AcceleoGeneratorTest : StringSpec({
    ...
    "Acceleo generation produces valid Java code for minimal hello".config(enabled = false) {
        ...
        Reflect.compile("com.plantestic.test.Test", File("$OUTPUT_PATH/scenario.java").readText()).create(MINIMAL_HELLO_CONFIG_PATH)
    }
    ...
    "Acceleo generation produces valid Java code for complex hello" {
        ...
        // Now compile the resulting code
        Reflect.compile("com.plantestic.test.Test", File("$OUTPUT_PATH/scenario.java").readText()).create(COMPLEX_HELLO_CONFIG_PATH)
    }
